...
static const int database_metatable;
static const int statement_metatable;
...
LUALIB_API int luaopen_sqlite3(lua_State *L)
{
  /* Create database metatable */
  lua_newtable(L);
  FUNC("__tostring", lua_database_tostring)
  FUNC("__gc", lua_database_gc)
  lua_pushliteral(L, "__index");
    lua_newtable(L);
    FUNC("prepare", lua_statement_prepare)
    FUNC("exec", lua_statement_exec)
  lua_rawset(L, -3);
  lua_rawsetp(L, LUA_REGISTRYINDEX, &database_metatable);
  /* Create statement metatable */
  lua_newtable(L);
  FUNC("__tostring", lua_statement_tostring)
  FUNC("__gc", lua_statement_gc)
  lua_pushliteral(L, "__index");
    lua_newtable(L);
    FUNC("bind", lua_statement_bind)
    FUNC("step", lua_statement_step)
    FUNC("reset", lua_statement_reset)
    FUNC("rows", lua_statement_rows)
  lua_rawset(L, -3);
  lua_rawsetp(L, LUA_REGISTRYINDEX, &statement_metatable);
  /* Create library */
  lua_newtable(L);
  FUNC("open", lua_database_open)
  return 1;
}
