...
#    sed '455s/.*/  -s/' ${NDK}/build/cmake/android.toolchain.cmake
    cmake -H. -B${BUILD_PATH} -DLUA_VERSION=5.3.5 -DLUAC_COMPATIBLE_FORMAT=ON -DANDROID_ABI=${ABI} -DCMAKE_BUILD_TYPE=Release -DCMAKE_TOOLCHAIN_FILE=${NDK}/build/cmake/android.toolchain.cmake -DANDROID_NATIVE_API_LEVEL=${API} -DANDROID_TOOLCHAIN=clang -DANDROID_TOOLCHAIN_NAME=${TOOLCHAIN_ANME}
